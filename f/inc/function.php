...
function get_info_url($id,$fid,$cityid='',$zoneid='',$streetid='',$array=array()){
	...
	if($webdb[Info_htmlType]==2){
		if($id){
			$url.="{$Fid_db[dir_name][$fid]}/{$id}x.$webdb[Info_htmlname]";	//信息页URL		
		}else{			
			if(!$zoneid&&!$streetid){
...
function Get_Info($type,$rows=5,$leng=20,$fid=0,$mid=0,$cityid='city',$zoneid='city',$streetid='city'){
	...
	if($type=='hot'){
		$SQL=" WHERE 1 $SQL";
		$SQL_ORDER=" ORDER BY rand() DESC LIMIT $rows ";
	}elseif($type=='lastview'){
		$SQL=" WHERE 1 $SQL ";
		$SQL_ORDER=" ORDER BY lastview DESC LIMIT $rows ";
	}elseif($type=='new'){
		$SQL=" WHERE  1 $SQL ";
		$SQL_ORDER=" ORDER BY id DESC LIMIT $rows ";
	}elseif($type=='level'){
	...
	$listdb=Info_list_content(" $SQL ",$SQL_ORDER,$leng=140,$fid_array,$_erp);
	return $listdb;
}
...
function seo_eval($string){
	global $city_DB,$fidDB,$city_id,$zone_id,$street_id,$zone_DB,$street_DB,$seo_tile;
	$string=str_replace(
		array('{city_name}','{zoon_name}','{street_name}','{sort_name}','{seo_tile}'),
		array($city_DB['name'][$city_id],$zone_DB['name'][$zone_id],$street_DB['name'][$street_id],$fidDB['name'],$seo_tile),
		$string);
	return $string;
}

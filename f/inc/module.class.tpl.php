...
function post_tpl($rs,&$tplcode){
	...
	if($rs[form_type]=='text')
	{
		...
		$show0="$mustfill{$rs[title]}:<br>{$rs[form_title]}";
	...
	elseif($rs[form_type]=='ieeditsimp')
	{
		...
		$show0="$mustfill{$rs[title]}:<br>{$rs[form_title]}";
		$show="	<script type='text/javascript' charset='utf-8' src='\$city_url/ewebeditor/3.1.1/kindeditor.js'></script>
<script type='text/javascript'>
    KE.show({
        id : 'atc_{$rs[field_name]}',
        cssPath : '\$city_url/ewebeditor/common.css',items : ['source', 'undo', 'redo', 'fontname', 'fontsize', 'textcolor', 'bgcolor', 'bold', 'italic', 'underline','removeformat', 'justifyleft', 'justifycenter', 'justifyright', 'insertorderedlist','insertunorderedlist','link', 'unlink']
    });
...
<script type='text/javascript'>
    KE.show({
        id : 'atc_{$rs[field_name]}',
        cssPath : '\$city_url/ewebeditor/common.css',items : ['source', 'preview', 'fullscreen', 'undo', 'redo', 'emoticons', 'cut', 'copy', 'paste',

...
$linkdb=array("地域/省份管理"=>"$admin_path&job=listsort","城市管理"=>"$admin_path&job=city","城市辖区管理"=>"$admin_path&job=zone","地段管理"=>"$admin_path&job=street","生成地区目录/二级域名城市设置"=>"$admin_path&job=batch");

if(!function_exists('MODULE_CK')||!in_array('fenlei',$BIZ_MODULEDB)){
	unset($linkdb["生成地区目录/二级域名城市设置"]);
}
...
elseif($job=="city")
{
	...
	//2013年8月5日 增加后台城市分页 原来没有这个功能的。增加了分页功能
	...
	$rows=500;
	$min=($page-1)*$rows;
	$showpage=getpage("{$_pre}city","$SQL","$admin_path&job=$job&fup=$fup",$rows);
	
	
	unset($sortdb,$infodb);
	$query = $db->query("SELECT * FROM {$_pre}city $SQL ORDER BY list DESC LIMIT $min,$rows");

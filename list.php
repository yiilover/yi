...
$totallist = mysql_num_rows(mysql_query("SELECT * FROM `{$pre}fenlei_content` WHERE fid=$fid"));

//SEO

if($page==1){//{$page2}栏目的当前页 $page有冲突
	$page2=" ";
	}else{
		$page2="第".$page."页";
		}
		
$titleDB[title]	= $fidDB[metatitle]?seo_eval($fidDB[metatitle]):strip_tags("{$city_DB[name][$city_id]} {$zone_DB[name][$zone_id]} {$street_DB[name][$street_id]} $fidDB[name] $seo_tile $page2");
...
function ListThisSort($rows,$leng){
	...
	while( $rs=$db->fetch_array($query) ){
		...
		$rs[content]=get_word($rs[full_content]=$rs[content],80);

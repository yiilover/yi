...
function write_file($filename,$data,$method="rb+",$iflock=1){
	...
	if(!$handle&&!strstr($filename,'cache/label_cache')){
		echo " <script>alert('此文件不可写:$filename')</script>";
	}
	if($iflock){
...
function getpage($table,$choose,$url,$rows=20,$total=''){
	...
	for($j=$s;$j<=$b;$j++){
		if($j==$page){
			$show.=" <a href='#' class=\"showbg\">$j</font></a>";
		}else{
			$show.=" <a href=\"$url&page=$j\" title=\"第{$j}页\">$j</a>";
		}
	}
	$showpage="<a href=\"$url&page=1\" title=\"首页\">首页</A> <a href=\"$url&page=$uppage\" title=\"上一页\">上一页</A>  {$show}  <a href=\"$url&page=$nextpage\" class=\"nextbg\" title=\"下一页\">下一页</A> <a href=\"$url&page=$totalpage\" title=\"尾页\">尾页</A> ";
    if($totalpage>1){
		return $showpage;
	}
}
...
function utf82gbk($text){
	...
	require_once(ROOT_PATH."inc/class.chinese.php");
	$cnvert = new Chinese("GB2312","BIG5",$text,ROOT_PATH."./inc/gbkcode/");

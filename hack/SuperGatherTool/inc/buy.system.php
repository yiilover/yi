...
$db ->query("
	...
	(`title`,`mid`,`fid`,`fname`,`hits`,`posttime`,`list`,`uid`,`username`,`picurl`,`ispic`,`yz`,`keywords`,`ip`,`begintime`,`endtime`,`city_id`,`passwd`,`picnum`,`price`) 
	VALUES
	('{$title}','{$fidDB[mid]}','{$fid}','{$fname}','{$hits}','{$posttime}','{$posttime}','{$uid}','{$username}','{$picurl}','{$ispic}','{$yz}','{$keywords}','{$onlineip}','{$begintime}','{$endtime}','{$city_id}','{$passwd}','{$picnum}','{$price}')
	");
...
if ($gather_module_valeDB[$key]) {
if (!$GLOBALS['IIIIIIIIIlII']("(gif|jpg|png|jpeg|bmp)$",$gather_module_valeDB[$key]) ||strlen($gather_module_valeDB[$key]) >= 100) {
continue;
}
$v = $GLOBALS['IIIIIIII11II'](strip_tags($gather_module_valeDB[$key],$restags));
$db ->query("INSERT INTO `{$_pre}pic` ( `id` , `fid` , `mid` , `uid` , `type` , `imgurl` , `name` ) VALUES ( '{$rs[id]}', '$fid', '{$fidDB[mid]}', '$uid', '0', '$v', '{$title}')");
}
}

<?php 
// 人才招聘模块 - 采集助手
// 功能：齐博 人才招聘模块 的采集通配符说明，实现采集规则助手功能
...
$Aarray[Mtable]="1.自动注册会员表:<font color='#0000FF'>{$pre}memberdata、{$pre}hy_company、{$pre}hy_company_fid </font> 2.求职招聘表:<font color='#0000FF'>{$_pre}content 、{$_pre}content_N </font>(其中N代表该模块下入库模型id)";  //入库涉及的表名
...
<table width='100%' border='1' cellpadding='0' cellspacing='0' bordercolor='#99CCFF' >
  ...
  <tr>
    <td colspan='3' bgcolor='#99CCFF'>以下为采集的招聘或人才模型字段通配符</td>
  </tr>
  <tr>
  ...
  <tr>
    <td><font color='#FF0000' title='点击即可实现复制' onClick='javascript:CopyText(this);'>{workplace=*}</font></td>
    <td>工作地点</td>
    <td>详细请根据入库模型自定义字段部分设置</td>
  </tr>
  <tr>
    <td><font color='#FF0000' title='点击即可实现复制' onClick='javascript:CopyText(this);'>{nums=*}</font></td>
    <td>招聘人数</td>
    <td>详细请根据入库模型自定义字段部分设置</td>
  </tr>
  <tr>
    <td><font color='#FF0000'>简历模型</font></td>
    <td>相关自定义字段</td>
    <td>参考简历模型中的自定义字段</td>
  </tr>
  <tr>

...
if ($city_id) {
	@extract($db -> get_one("SELECT fup AS provinceid,name AS city FROM {$pre}area WHERE fid='$city_id'"));
	@extract($db -> get_one("SELECT name AS province FROM {$pre}area WHERE fid='$provinceid'"));
} else {
	$city_id = $city = $province = $provinceid = '';
} 

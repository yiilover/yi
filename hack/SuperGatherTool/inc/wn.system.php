...
$fidDB=$db->get_one("SELECT A.*,B.config AS m_config FROM {$_pre}sort A LEFT JOIN {$_pre}module B ON A.mid=B.id WHERE A.fid='$fid'");
$db->query("INSERT INTO `{$_pre}content_$fidDB[mid]` ( `id`, `fid`, `uid`, `topic`, `content`) VALUES ('$rs[id]','$fid','$uid','0','$content')");
@$GLOBALS['IIIIIIII1lI1']($db->get_one("SELECT rid FROM `{$_pre}content_$fidDB[mid]` WHERE id='$rs[id]' ORDER BY rid DESC LIMIT 1 "));
}
elseif(!$ForbidAdd)
{
$fidDB=$db->get_one(" SELECT A.name,B.config,A.mid FROM {$_pre}sort A LEFT JOIN {$_pre}module B ON A.mid=B.id WHERE A.fid='$fid' ");
$fname=$fidDB[name];
...
$db->query("
	...
	( `title`, `mid`,  `fid`, `fname`, `pages`, `posttime`, `list`, `uid`, `username`,`copyfrom`, `copyfromurl`,  `picurl`,`ispic`, `yz`, `keywords`, `jumpurl`, `iframeurl`, `ip`,`author`,`hits`,`cityid` , `city` , `provinceid` , `province`  ) 
	VALUES
	('$title','$fidDB[mid]','$fid','$fname','1','$posttime','$posttime','$uid','$username','$copyfrom','$curl','$picurl','$ispic','$yz','$keywords','$jumpurl','$iframeurl','$onlineip','$author','$hits','$city_id','$city','$provinceid','$province')
	");
$rs=$db->get_one("SELECT id FROM `{$_pre}content` ORDER BY id DESC LIMIT 1");
$db->query("INSERT INTO `{$_pre}content_$fidDB[mid]` ( `id`, `fid`, `uid`, `topic`, `content`) VALUES ('$rs[id]','$fid','$uid','1','$content')");
if($fidDB[config]){
$SQL='';
$M_config=$GLOBALS['IIIIIIII1l1I']($fidDB[config]);
foreach( $M_config[field_db] AS $key=>$value){
if($gather_module_valeDB[$key] &&$key!='content'){
$v=$GLOBALS['IIIIIIII11II']($gather_module_valeDB[$key]);
$SQL.=",`$key`='$v'";
}
}
if($SQL){
@$GLOBALS['IIIIIIII1lI1']($db->get_one("SELECT rid FROM {$_pre}content_$fidDB[mid] ORDER BY rid DESC LIMIT 1 "));
$db->query(" UPDATE `{$_pre}content_$fidDB[mid]` SET uid='$uid' $SQL WHERE rid='$rid'");
}
}
}


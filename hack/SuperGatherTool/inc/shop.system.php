...
if (!$fidDB[fid]) {
showerr('你没有选择数据要入库的栏目 ');
}
if ($fidDB[type] == 1) {
showerr('你不能把数据采进大分类,只能采进小栏目');
}
$fname = $fidDB[name];
...
$db ->query("
	...
	(`title`,`mid`,`fid`,`fname`,`hits`,`posttime`,`list`,`uid`,`username`,`picurl`,`ispic`,`yz`,`keywords`,`ip`,`endtime`,`city_id`,`picnum`,`price`) 
	VALUES
	('{$title}','{$fidDB[mid]}','{$fid}','{$fname}','{$hits}','{$posttime}','{$posttime}','{$uid}','{$username}','{$picurl}','{$ispic}','{$yz}','{$keywords}','{$onlineip}','{$endtime}','{$city_id}','{$picnum}','{$price}')
	");

...
//SEO
$titleDB[title]			= filtrate(strip_tags("$rsdb[title] - {$city_DB[name][$city_id]}$fidDB[name] {$zone_DB[name][$zone_id]} - $webdb[webname]"));
$titleDB[keywords]		= filtrate(strip_tags("{$city_DB[name][$city_id]}$fidDB[name],$fidDB[name],{$webdb[webname]}$fidDB[name]"));
...
/**/
function substr_b ($str,$a,$b){
	   $strdb = substr($str,$a,$b);
	   return $strdb;
	}
?>